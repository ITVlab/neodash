 ...
 * <p>
 * Subclasses MUST call the {@link #listenTo(Set)} method to start listening
 * for extension updates, and MUST call {@link #destroy()} when the instance is not
 * used anymore.
 * <p>
 * Subclasses should implement {@link #onExtensionDataChanged(ComponentName)}
 * in order to receive updates for the registered extensions passed to
 * {@link #listenTo(Set)}.<br/>
 * <p>
...
public abstract class DashClockHost {
     ...
     *
     * @see #isMultiplexerServicePresent(Context)
    ...
    /**
     * Return an {@link Intent} reference to redirect the user
     * to the Play Store to download the official DashClock app.<br/>
     * Implementers should call to {@link Context#startActivity(Intent)}.
     *
     * @return The download {@link Intent} or {@code null} if the the multiplexer
     ...
     *
     * @see #getOtherAppsWithReadDataExtensionsPermission(Context)
     */
